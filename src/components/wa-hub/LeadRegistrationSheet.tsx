...
import { type LeadSource, DEFAULT_AI_SDR_CONFIG } from '@/types/ai-sdr';
  ...
  const createLeadAndTrigger = async (lead: LeadInput, sourceId: string, shouldTriggerAi: boolean): Promise<boolean> => {
    ...
    try {
      ...
      if (existingConv) {
        ...
        if (lead.linkedinUrl) convUpdate.linkedin_profile = { url: lead.linkedinUrl };
        ...
        const { data: newConv, error: convErr } = await supabase
          .from('wa_conversations').insert({
            ...
            linkedin_profile: lead.linkedinUrl ? { url: lead.linkedinUrl } : null,
