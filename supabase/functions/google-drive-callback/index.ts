...
Deno.serve(async (req) => {
  try {
    ...
    // Store tokens in google_drive_tokens table
...
function redirectHtml(error: string | null): string {
  const message = error ? `Erro: ${error}` : "Google Drive conectado com sucesso!";
  const appUrl = error ? "/" : "/?gdrive=success";
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Google Drive</title></head><body>
    <p>${message}</p>
    <p>Redirecionando...</p>
